...
const rooms = new Map(); // roomCode -> Map(peerId -> { ws, userId, state })
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (data.type === 'join_room') {
        ...
        rooms.get(currentRoom).set(peerId, { ws, userId, state: data.playerState });

        // Send assigned peerId to client
        ws.send(JSON.stringify({ type: 'init', peerId, userId, room: currentRoom }));
