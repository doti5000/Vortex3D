...
export function createAuthModal({ onAuthSuccess }) {
  ...
  function render() {
    modal.innerHTML = `
      <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; background: #070b19; border-bottom: 1px solid #1e293b;">
        <div>
          ...
          <span style="font-size: 11px; color: #94a3b8; font-weight: 600;">Server-Verified Zero-Trust Authentication</span>
        </div>
    ...
    modal.querySelector('#btn-phryco-sso').addEventListener('click', () => {
      ...
      const authUrl = `https://phryco.vercel.app/pages/sso/authorize.html?client_id=${PHRYCO_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&code_challenge_method=plain&code_challenge=${code_verifier}&scope=profile email avatar`;
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        backdrop.remove();
        alert(`🎉 Welcome ${data.user.username}! Server authenticated successfully.`);
      } catch (err) {
        errBox.textContent = 'Backend authentication server connection error. Client-side fallbacks are disabled for security.';
        errBox.style.display = 'block';
      }
    });
  }
