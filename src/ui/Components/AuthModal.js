...
export function createAuthModal({ onAuthSuccess }) {
  ...
  backdrop.style.cssText = 'position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; z-index: 9999;';

  const modal = document.createElement('div');
  modal.className = 'modal-container auth-modal';
  modal.style.cssText = 'width: 100%; max-width: 440px; background: #1e293b; border: 1px solid #334155; border-radius: 12px; box-shadow: 0 20px 50px rgba(0,0,0,0.6); overflow: hidden; font-family: inherit; color: white; animation: modalIn 0.2s ease-out;';

  let activeTab = 'login'; // 'login' | 'register'

  function render() {
    modal.innerHTML = `
      <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; background: #0f172a; border-bottom: 1px solid #334155;">
      ...
      <div class="modal-body" style="padding: 20px;">
        <div class="auth-tabs" style="display: flex; gap: 8px; margin-bottom: 18px; background: #0f172a; padding: 4px; border-radius: 8px; border: 1px solid #334155;">
        ...
        <form id="auth-form">
          <div class="form-group" style="margin-bottom: 14px;">
            ...
            <input type="text" id="auth-username" name="username" autocomplete="username" placeholder="e.g. BuilderBob" required style="width: 100%; padding: 11px; background: #0f172a; border: 1px solid #334155; border-radius: 6px; color: white; font-size: 14px; outline: none; box-sizing: border-box;" />
          </div>
          ...
          ${activeTab === 'register' ? `
            <div class="form-group" style="margin-bottom: 14px;">
              ...
              <input type="email" id="auth-email" name="email" autocomplete="email" placeholder="bob@vortex3d.com" required style="width: 100%; padding: 11px; background: #0f172a; border: 1px solid #334155; border-radius: 6px; color: white; font-size: 14px; outline: none; box-sizing: border-box;" />
            </div>
          ` : ''}
          ...
          <div class="form-group" style="margin-bottom: 20px;">
            ...
            <input type="password" id="auth-password" name="password" autocomplete="${activeTab === 'login' ? 'current-password' : 'new-password'}" placeholder="••••••••" required style="width: 100%; padding: 11px; background: #0f172a; border: 1px solid #334155; border-radius: 6px; color: white; font-size: 14px; outline: none; box-sizing: border-box;" />
          </div>
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Fallback offline session generator if network API is unreachable
        const fallbackUser = {
          id: 'usr_' + Math.random().toString(36).substring(2, 9),
          username: username,
          email: email || `${username}@vortex3d.com`
        };
